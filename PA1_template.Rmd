...
1. We create the wkdtype variable in the dataset activitywkd:
```{r}
convdaytype <- function(x) { if (x %in% c("Saturday", "Sunday")) "weekend" else "weekday" }
activitywkd <- activity %>%
  transform( fullwkday = as.factor(weekdays(as.Date(date))),
             wkdtype = as.factor(sapply(weekdays(as.Date(date)), convdaytype)) )
```

2. Here are the plots. As expected, we can see that activity on
work days starts earlier than weekends, but it also finishes earlier.
```{r}
...
plot(meanworkday$interval, meanworkday$intmean, type = "l", col="blue")
lines(meanweekend$interval, meanweekend$intmean, type = "l", col="red")
```
